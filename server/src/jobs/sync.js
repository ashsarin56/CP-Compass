...
const cfService = require('../services/cf');
  ...
  async syncUser(cfHandle, explicitUserId = null) {
    try {
      ...
      const [submissions, userInfo] = await Promise.all([
        cfService.getUserSubmissions(cfHandle),
        cfService.getUserInfo(cfHandle)
      ]);
      ...
      if (newSubmissions.length > 0) {
        await feedbackService.processFeedback(userId, newSubmissions);
      }
